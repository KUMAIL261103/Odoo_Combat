...
export const Maintenance = () => {
  ...
  return (
    <>
      ...
      <div className="flex  flex-col  justify-center items-center min-h-screen border rounded-xl">
        {facility.map((facility, index) => (
          <div key={index} className="border-2 rounded-md my-2 w-3/4">
            <div className="bg-light-green flex justify-around items-center w-full">
              <img
                src={`${facility.image}`}
                alt="img"
              ...
              <div>
                <button
                  className={`bg-light-green text-black py-2 px-4 rounded mx-2 ${
                    user.role === "manager" ? "visible" : "hidden"
                  }`}
                  onClick={() => {
                ...
                >
                  {" "}
                  {togglelogs.includes(index) == true
                    ? "Show less"
                    : "Show logs"}
                </button>
              </div>
            ...
            <div
              className={`flex flex-col items-center ${
                togglelogs.includes(index) == true ? "visible" : "hidden"
              }`}
            >
              {facility.MaintenanceId.length == 0 ? (
                <div className="text-black">No logs available</div>
              ) : null}
              {facility.MaintenanceId?.map((maintenance, index) => (
                <div
                  className="bg-slate-800 text-white p-4 w-full max-w-md rounded-lg mt-4"
                  key={index}
                >
